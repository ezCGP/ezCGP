...
class DataHistoryBuffer:
    ...
    def __init__(self, shape, max_size, batch_size):
        ...
        self.history_buffer = np.zeros(
            (0, *shape)
        )  # We use 0 in the first axis because we will be appending until we reach the max size
    ...
    def add(self, data_points, nb_to_add=None):
        ...
        if len(self.history_buffer) < self.max_size:
            ...
            self.history_buffer = np.append(
                self.history_buffer, data_points_copy[:nb_to_add], axis=0
            )
...
class SimGANDataset:
    ...
    def __init__(
        self, real_size=512, sim_size=128**2, batch_size=128, buffer_size=12800
    ):
        self.batch_size = batch_size
        ...
        self.real_raw = self.gen_fake_dataset(
            real_size,
            [24, 56],
            [0.5, 0.5],
            [5, 8],
        ...
        self.simulated_raw = self.gen_fake_dataset(
            sim_size,
            [23.5, 55.5],
            [0.5, 0.5],
            [5, 10],
        ...
        self.data_history_buffer = DataHistoryBuffer(
            (1, self.real_raw.shape[-1]), buffer_size, batch_size
        )
        ...
        assert (
            real_channels == sim_channels
        ), "Something wrong with shape of data...mismatch number of channels"
        assert (
            real_length == sim_length
        ), "Something wrong with shape of data...mismatch length of data"
    ...
    def gen_fake_dataset(
        ...
        for i in range(0, len(peak_locs_mu)):
            peaks[i, :] = np.random.normal(
                loc=peak_locs_mu[i], scale=peak_locs_sigma[i], size=n_signals
            )
            sigmas[i, :] = np.random.normal(
                loc=mu_sigma[i], scale=peak_locs_sigma[i], size=n_signals
            )
            g_amplitudes[i, :] = np.random.normal(
                loc=amp_mu[i], scale=amp_sigma[i], size=n_signals
            )
        ...
        for i in range(0, len(freq_mu)):
            temp_f.append(
                np.array(
                    [
                        np.random.normal(loc=m, scale=s, size=n_signals)
                        for m, s in zip(freq_mu[i], freq_sigma)
                    ]
                )
            )
        ...
        for i in range(0, n_signals):
            primary, residual = self.gen_siganl(
                peaks[:, i], sigmas[:, i], g_amplitudes[:, i], freqs[i]
            )
        ...
        if add_noise:
            ...
            for i in range(len(data)):
                for j in range(len(data[i][0])):
                    data[i][0][j] += np.random.normal(0, 0.05) * np.clip(
                        data[i][0][j], 0.5, 1
                    )
            # Normalize
            for i in range(len(data)):
    ...
    def gen_siganl(
        self, peaks, sigmas, g_amplitudes, freqs, f_amplitudes=0.02, fs=1 / 60, size=92
    ):
        """
    ...
    def gen_gaussian_kernel(self, sigma, mu, size=92):
        ...
        kernel = (1 / (np.sqrt(2 * np.pi) * sigma**2)) * np.exp(
            -0.5 * (((x - mu) ** 2) / sigma**2)
        )
