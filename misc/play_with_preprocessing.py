...
for name, execute in inspect.getmembers(globals()[ops]): # returns list of tuples of everything in that module
    if (inspect.isfunction(execute)) and  (execute.__module__.endswith(ops)):
        ...
        # build out args
        args = []
        for arg_type in ops.operator_dict[operation]["args"]:
...
def mimic_keras_preprocess_func(pipeline, image):
    ...
    for operation in pipeline.operations:
        print("Executing %s" % operation.__name__)
...
mimic_keras_preprocess_func(pipeline, images)