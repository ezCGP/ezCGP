...
x_train = [np.random.uniform(low=0.25, high=2, size=200)] #np.float64(-1),
y_train = goalFunction(x_train[0])
...
skeleton_block = { #this skeleton defines a SINGLE BLOCK of a genome
    ...
    'setup_dict_arg': {
        ...
        arguments.argBool: {'prob': 1},
        arguments.argFloat: {'prob': 1},
    ...
    'block_input_dtypes': [np.ndarray], #[np.float64, np.ndarray], #placeholder datatypes so that the genome can be built off datatypes instead of real data
...
skeleton_genome = { # this defines the WHOLE GENOME
    'input': [np.ndarray], #[np.float64, np.ndarray],
    'output': [np.ndarray],
    1: skeleton_block
}
