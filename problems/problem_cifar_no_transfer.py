...
class Problem(ProblemDefinition_Abstract):
    ...
    def __init__(self):
        ...
        mpi = False
        #genome_seeds = []
        genome_seeds = glob.glob("outputs/problem_cifar_no_transfer/%s/univ0000/gen_%04d_*.pkl" % ("20210308-081111-third_run", 2))
        if len(genome_seeds) == 0:
            print("using gen01 instead of gen02")
            genome_seeds = glob.glob("outputs/problem_cifar_no_transfer/%s/univ0000/gen_%04d_*.pkl" % ("20210308-081111-third_run", 1))
