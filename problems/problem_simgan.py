...
from codes.block_definitions.shapemeta.block_shapemeta import (
    BlockShapeMeta_SimGAN_Network,
    BlockShapeMeta_SimGAN_Train_Config,
)
from codes.block_definitions.operators.block_operators import (
    BlockOperators_SimGAN_Refiner,
    BlockOperators_SimGAN_Discriminator,
    BlockOperators_SimGAN_Train_Config,
)
from codes.block_definitions.arguments.block_arguments import BlockArguments_Auto
from codes.block_definitions.evaluate.block_evaluate_pytorch import (
    BlockEvaluate_SimGAN_Refiner,
    BlockEvaluate_SimGAN_Discriminator,
    BlockEvaluate_SimGAN_Train_Config,
)
from codes.block_definitions.mutate.block_mutate import (
    BlockMutate_OptB_No_Single_Ftn,
    BlockMutate_OptB,
    BlockMutate_ArgsOnly,
)
from codes.block_definitions.mate.block_mate import BlockMate_WholeOnly
from codes.individual_definitions.individual_mutate import (
    IndividualMutate_RollOnEachBlock_LimitedMutants,
)
...
class Problem(ProblemDefinition_Abstract):
    ...
    def __init__(self):
        ...
        discriminator_def = self.construct_block_def(
            nickname="discriminator_block",
            shape_def=BlockShapeMeta_SimGAN_Network,
            operator_def=BlockOperators_SimGAN_Discriminator,
            argument_def=BlockArguments_Auto(
                BlockOperators_SimGAN_Discriminator().operator_dict, 15
            ),
        ...
        train_config_def = self.construct_block_def(
            nickname="train_config",
            shape_def=BlockShapeMeta_SimGAN_Train_Config,
            operator_def=BlockOperators_SimGAN_Train_Config,
            argument_def=BlockArguments_Auto(
                BlockOperators_SimGAN_Train_Config().operator_dict, 10
            ),
        ...
        self.construct_individual_def(
            block_defs=[refiner_def, discriminator_def, train_config_def],
    ...
    def construct_dataset(self):
        ...
        train_config_dict = {"device": "cpu"}  # was gpu but that didn't work anymore
        self.training_datalist = [
            simganData.SimGANDataset(real_size=512, sim_size=128**2, batch_size=128),
            train_config_dict,
        ]
        self.validating_datalist = [
            simganData.SimGANDataset(
    ...
    def population_selection(self, universe):
        for i, indiv in enumerate(universe.population.population):
            ezLogging.warning(
                "Final Population Scores: (%i) %s %s"
                % (i, indiv.id, indiv.fitness.values)
            )
    ...
    def save_pytorch_individual(self, universe, original_individual):
        ...
        """
        ezLogging.debug(
            "Saving individual %s from generation %i"
            % (original_individual.id, universe.generation)
        )
    ...
    def postprocess_generation(self, universe):
        ...
        auc = plot_things.plot_pareto_front(
            axes[0, 0],
        ...
        plot_things.plot_save(
            fig, os.path.join(universe.output_folder, f"ting{universe.generation}.jpg")
        )
